...
- 修复治理目录 README 中变更日志归档行的编码污染，并统一归档文档中文标题。
- 增强体积治理护栏：根 `CHANGELOG.md` 必须继续链接详细归档，变更日志详细流水归档按日期或关键词读取。
- 将变更日志详细流水归档纳入条件大文件清单、`00-index` 顶部大文件清单和默认路线泄漏检查。
...
- 收紧状态元数据检查脚本：目录 README 不再被跳过，分类索引也必须保留明确维护状态。
- 对齐状态元数据检查说明和脚本报错文案，明确目录 README 也属于需要维护状态的正式入口。
- 统一主要目录 README 顶部的 `状态` 与 `用途` 元数据格式，并补齐 `scene-craft` 分类入口状态。
- 为目录 README 顶部 `用途` 元数据增加可消费性检查，避免分类索引退回无路由说明。
