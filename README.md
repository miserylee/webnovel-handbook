...
- 统一命令入口：[package.json](package.json)，常用命令为 `npm run check`、`npm run package-skill`、`npm run sync-skill:check` 和 `npm run sync-skill`。
- 调整索引、路径或 skill 入口后，可运行 `node scripts/check-handbook-consumability.mjs` 检查 Markdown 本地链接、本仓库 `docs/...md` 裸路径、目录 README 覆盖、同一 `docs` 分类目录内编号唯一、文件名编号与标题编号一致、正式文档状态元数据、关键 agent 入口与用途信号、`docs` 文件名跨平台命名、`docs` 一级目录索引覆盖、skill 包布局、skill 启动最小阅读包、skill 打包设置、活跃入口体积护栏、超大 Markdown 白名单、UTF-8 BOM、CRLF 换行、常见编码污染和开源去身份化残留。
