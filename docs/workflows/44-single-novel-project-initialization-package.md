...
- 已完成或暂定 `docs/workflows/43-commercial-project-greenlight-scorecard.md` 的立项判断。
- 至少明确目标平台或候选平台。
- 至少明确目标读者或候选目标读者。
- 至少明确主承诺。
- 至少有一个可验证的第一章入口。
...
- `候选` 不得进入正文成为事实。
- `待确认` 不得作为章节任务的前提。
- `已确认` 变更必须写入 `docs/governance/99-decision-log.md`。
...
- 是否有第一章任务单？
- 是否写明不能进入正文的情况？
- 是否把关键决策写入 `docs/governance/99-decision-log.md`？
