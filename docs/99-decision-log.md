...
## 2026-06-10：降 AI 味提升为章节交付硬门槛

状态：已确认

背景：用户反馈在当前规范下写出的稿子 AI 味仍偏重，说明“降 AI 味”在执行链里容易被当成局部润色或可选检查项。

决策：降 AI 味提升为写稿、试读、审稿、仲裁和修订中的高优先级质量门槛。高 AI 味或极高 AI 味稿件不能以“整体通顺”为理由交付；必须先修人物自保动作、场景摩擦、因果压力、对话反应、具体动作链和情绪触发物。

执行规则：

- `skills/webnovel-handbook/SKILL.md` 中，写稿、续写、审稿和改稿任务必须读取 `docs/06-ai-writing-guidelines.md` 与 `docs/11-human-writing-upgrade.md`。
- `docs/58-integrated-drafting-beta-review-revision-workflow.md` 中，反 AI 味试读结果为 `高` 或 `极高` 时，反馈仲裁必须列入“必须修改”。
- 修订时不先做词句替换，而是先修人物、场景、因果、动作和声音。
- 有效粗糙可以保留；平均正确、全员同声、情绪靠旁白、场景像纪要的文本必须改。

## 2026-06-10：逻辑连续性提升为章节交付硬门槛

状态：已确认

背景：用户反馈部分稿件出现明显逻辑漏洞、前后逻辑错位，或莫名其妙冒出没有理由的句子。说明现有流程对“读者能否沿着因果读下去”的检查不够前置，容易把逻辑问题留到终稿。

决策：逻辑连续性与句子功能提升为写稿交付硬门槛。明显逻辑漏洞、信息空降、动作顺序错误、角色知识越界和无功能句，不能以“文笔顺”或“氛围好”为理由保留。

执行规则：

- 写稿前必须确认上一章/上一场承接、本场因果链、新信息来源、角色已知/未知和关键动作顺序。
- 试读环节增加逻辑冷读者视角，专查前后错位、突兀信息、动作顺序错误和无理由句子。
- 反馈仲裁中，逻辑连续性硬门槛排在主承诺和目标读者痛点之前。
- 修订时先修因果、信息来源、角色知识和句子位置，再做文风润色。
- 如果一句话不能解释它在动作、信息、人物、情绪、节奏或钩子中的功能，优先删除、移位或补因果。

## 2026-06-10：审稿引导增加反例

状态：已确认

背景：用户要求在审查引导中增加更多反例。单纯写“要审节奏、人物、AI 味、逻辑”仍可能导致 agent 输出空泛审稿话术，例如“节奏加强”“人物更鲜活”“润一下 AI 味”，但不能指导改稿。

决策：在 `docs/08-review-and-reader-feedback.md` 与 `docs/58-integrated-drafting-beta-review-revision-workflow.md` 中增加审稿反例，明确哪些审稿输出本身不合格，并给出对应的正确审查动作。

执行规则：

- 审稿建议不能停留在“加强、优化、丰满、自然一点、再打磨”。
- 每个主要问题必须绑定证据位置、读者影响和可执行动作。
- 审稿反例优先覆盖空泛审稿、误把通顺当合格、AI 味词句化处理、逻辑跳跃放行、漂亮但无功能句保留、作者脑补替代文本证据等高频错误。
- 除审稿话术反例外，还需要保留稿文命中反例，让 agent 能直接识别坏稿片段中的逻辑跳跃、信息空降、角色知识越界、动作顺序错误、无功能句、AI 味旁白、全员同声、证据链纪要、假钩子、主角被推走、反派降智和爽点未落袋。


## 2026-06-10：章节字数与细纲门槛

- 状态：已确认。
- 决策：普通可发布中文网文章节默认约 3000 字，常规目标区间为 2800-3500 字。低于 2500 字时优先检查细纲是否过薄；低于 2000 字默认视为片段或短稿，除非用户明确要求短章。
- 执行：如果章节太短，先补细纲中的场景目标、阻力、失败尝试、后果、情绪余波、阶段回报和章末压力，再扩正文，不用水文凑字。

## 2026-06-10：成片短句属于 AI 味高风险

- 状态：已确认。
- 决策：写稿时不禁止短句，但禁止连续堆叠同长度、同语法、同语气的短句。短句只应服务打击、转折、惊吓、喜剧落点、动作急促或人物失控，不能成为普通叙述默认纹理。
- 执行：起草 agent 需要主动做长短句变化和动作链承接；试读与审稿 agent 必须识别机械断句、提纲残留和情绪口号；修订 agent 必须先合并无功能短句，再补现场反馈、人物遮掩、关系压力或具体物件。

## 2026-06-11：番茄投稿资料必须前置准备

- 状态：已确认。
- 决策：目标平台为番茄时，AI agent 在开书包或投稿前必须提前准备书本名称、目标读者、作品标签、主角名、作品简介和封面方案，不能只准备正文。
- 执行：书名按截图可见 `15` 字以内准备，主角名按 `5` 字以内准备，简介准备 `50-500` 字并先做合规检查；封面按截图可见 `600*800`、`jpg/png/jpeg`、不超过 `5MB` 和本地上传审核 `1-2` 小时预留处理。
- 边界：平台后台字段可能变化，正式投稿前必须以当前后台和官方规则为准。

## 2026-06-11：顶尖网文短摘句可作为对白示例

- 状态：已确认。
- 背景：用户指出 AI 写稿在人物说话上很苦手，需要摘录一些别人写的小说片段作为指导。
- 决策：公开仓库可以保留单句级短摘句，用于说明人物对白的场景功能、说话策略、关系杠杆和句子形状；不得扩展成连续原文库。
- 执行：新增顶尖网文短摘句示例和拆解卡。Agent 使用时只学习功能，目标作品台词必须原创改写，不能直接复制短摘句。
- 原因：人物对白质量需要高水平样本校准；开源知识库的价值是把样本转成方法、流程和检查项。
## 2026-06-12：人物对白自然度优先于金句感

- 状态：已确认。
- 背景：用户指出人物说话训练最重要的是自然，不是摘句、名句或压场句。
- 决策：对白质量判断的第一门槛是自然度。台词必须先符合人物当前关系、压力、知识范围、遮掩内容、情绪强度和说话习惯，再追求锋利、好笑、压场或商业记忆点。
- 执行：起草 agent 写对白前必须判断“这个人此刻敢说到几分”；审稿 agent 先拦截解释腔、总结腔、心理咨询腔、设定说明腔和金句硬塞；修订 agent 先把台词改成角色能自然说出口的版本，再打磨节奏和力度。
- 原因：不自然的金句会让读者立刻看到作者和 AI；自然但略平的台词仍可继续打磨，前者必须重写。

## 2026-06-12：试写纠错经验必须沉淀为可复用规则

- 状态：已确认。
- 背景：用户要求通过多轮 100 字以内小片段试写纠错，让 agent 学习并把经验记下来。本轮确认了现实向家庭对峙和职场小反击中的四类问题。
- 决策：试写纠错不能只停留在聊天里。凡是用户明确指出并通过改写验证的问题，都应沉淀为“坏句 -> 用户反馈 -> 问题原因 -> 通过版 -> 可迁移规则”。
- 已确认经验：不要用散文化物象硬压现实情绪；熟人台词要口语但不能碎成短句；自然对白常有拖带、重复、半句转折和具体物件；功能性动作不能太直球，尤其录音、截图、证据、合同等动作要先像人物顺手做的事。
- 执行：新增 `docs/04-character-and-dialogue.md` 的试写纠错样本、新增 `docs/06-ai-writing-guidelines.md` 的“动作不要直球报功能”、新增 `docs/10-templates-and-checklists.md` 的小片段试写纠错沉淀卡。

## 2026-06-12：职场对峙对白要保留试探和关系压力

状态：已确认

用户在试写纠错中确认：自然对白的关键不是“更简洁”，而是更像真实人物在具体关系里说话。职场对峙不能写成审讯台；老板的压迫应体现为看似随口的甩锅、试探和追问，主角的反击应保留表面客气、半句绕法和实际威胁。

执行规则：

1. 老板、领导、甲方等上位者不要直接替作者说结论，优先使用“好像不太对”“你瞅瞅”“昨天是你跟的吗”等带试探的口语。
2. 主角自证时要有现实中的自保语气，避免像汇报材料或法庭陈述。
3. 高光反击不要只追求锐利，要保留关系场里的遮掩、礼貌和反问。
4. 审稿时遇到“空气静了”“笑容僵住”“谁改的我知道”等模板化反应，优先改成人物继续装正常、继续试探或借话反压。

已写入 `docs/04-character-and-dialogue.md`。
## 2026-06-12：试写纠错必须保留 Agent 初稿和用户改稿

状态：已确认

用户确认：试写纠错样本不能只记录最终通过版，必须把 Agent 写的版本和用户改的版本都记录下来，作为对比案例。这样后续 agent 才能看见 AI 味的来源、用户改动的方向，以及“自然”和“像人写”到底体现在哪些具体词句上。

执行规则：

1. 每次用户给出可学习的改稿，都要保留 Agent 初稿、用户改稿、差异分析和可迁移规则。
2. 不把用户改稿只抽象成原则；具体句子本身也是训练资料。
3. 对比案例优先写进 `docs/04-character-and-dialogue.md`，模板写进 `docs/10-templates-and-checklists.md`。
4. 后续审稿或写稿时，遇到“AI 味重”“不像人写”，优先回看这些成对案例。
## 2026-06-12：样本对比单独整理为参考书

状态：已确认

用户确认：对话试写纠错中的样本对比，不应散落在人物与对话专题里，应单独整理成一篇参考书，方便后续 agent 在写稿和审稿时直接查阅“Agent 初稿 / 用户改稿 / 差异分析 / 可迁移规则”。

执行规则：

1. 新增 `docs/59-dialogue-comparison-reference.md` 作为样本对比参考书。
2. `docs/04-character-and-dialogue.md` 保留原则和入口，不再承载完整对比案例。
3. README 和 `docs/00-index.md` 将该文档加入起草、审稿和人物对话相关入口。
4. 后续用户纠错形成的高价值样本，优先追加到该参考书，而不是只写进普通专题。
## 2026-06-13：封面标准只沉淀通用判断，不保留单次生成方案

状态：已确认

用户确认：封面指导手册需要抽出通用设计标准，不能把具体某本书的书名、人物设定、风格方向和单次生成方案当作通用规则。失败封面复盘中可迁移的是“不要给图贴大字”“标题是商品招牌”“底图和标题共同服务类型承诺”“封面要接受点击测试”等判断。

执行规则：

1. `docs/17-cover-visual-packaging.md` 增补商业网文封面设计标准，强调平台合规、缩略图可读、类型承诺和点击理由。
2. 单本项目的具体封面方向、标题拆行、人物设定和版本编号留在项目文档，不进入公共知识库通用规则。
3. 番茄多封面效果排名可转化为“A/B 候选与点击假设”规则，但不把番茄后台某一时点功能升格为所有平台规则。
4. 正式上传前仍需重新确认目标平台最新尺寸、格式、大小、审核和修改规则。
## 2026-06-13：AI 封面文字优先一体生成，不用脚本硬贴字

状态：已确认

用户确认：封面标题不要默认用脚本生成字体，因为容易很丑、很廉价。封面文字应优先让 AI 作为画面整体的一部分直接绘制出来，标题字、人物、背景和光影要共同构成商业网文封面，而不是先做底图再贴大字。

执行规则：

1. 正式封面优先采用 AI 一体生成标题字和底图的方案。
2. 脚本排字只允许作为临时占位、尺寸预览或兜底，不作为推荐正式流程。
3. AI 画字必须逐字检查，错字、漏字、变形和缩略图不可读时应重生成，不用丑字补丁强救。
4. `docs/17-cover-visual-packaging.md` 中如有旧流程和本规则冲突，以本规则为准。
## 2026-06-13：AI 新建文件必须同步建立索引路径

状态：已确认

用户确认：AI 在项目中创建的所有文件，必须有明确索引路径，不能让后续 agent 找不到。没有入口的新文件会破坏长篇项目和知识库的连续性，即使内容本身有价值，也不能视为合格交付。

执行规则：

1. AI 创建新文件时，必须同步更新 `README.md`、`docs/00-index.md`、目录 README、台账、决策记录或相关专题入口中的至少一个。
2. 交付说明必须列出新文件路径、索引入口、读取场景和维护状态。
