...
- `docs/governance/99-decision-log.md`：活跃决策短索引；历史长文按月归档到 `docs/decisions/`。`docs/governance/01-changelog-detail-archive.md` ????????????????????
...
`docs/governance/99-decision-log.md` 是活跃决策短索引，适合默认读取。`docs/decisions/` 是历史决策归档，只在需要追溯旧决策原因时按月份和关键词检索。`docs/governance/01-changelog-detail-archive.md` 是根 `CHANGELOG.md` 下沉出来的详细维护流水，只在追溯具体变更细项、排查历史维护动作或准备发布说明时按日期/关键词读取。
