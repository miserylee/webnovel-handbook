...
状态：已确认  
用途：保存从根 `CHANGELOG.md` 下沉的详细维护流水。本文不是默认启动入口，只在追溯具体变更细项、排查历史维护动作或准备发布说明时按日期/关键词检索。

## 读取规则

- 默认只读根 `CHANGELOG.md` 的概要。
- 需要具体维护细项时，再按关键词、日期或小节读取本文。
- 本文只保存开源可见的维护流水，不记录私有项目名、本机路径、账号、群名或可识别个人的信息。
- 根 `CHANGELOG.md` 应保持短小，新增大量细项时优先继续下沉到本文或后续归档文件。

## 归档内容

以下内容为 2026-06-15 前从根 `CHANGELOG.md` 下沉的详细流水。
